...
| VM | `income-api` (`koreacentral`, `Standard_B2als_v2`) |
...
```powershell
az vm create --resource-group rg-income-lab --name income-api --location koreacentral `
  --image Ubuntu2204 --size Standard_B2als_v2 --admin-username azureuser --generate-ssh-keys
...
VM dùng system-assigned Managed Identity với role `Storage Blob Data Reader` trên Storage
Account. File service chỉ cần hai biến không bí mật: `ARTIFACT_BUCKET=income-lab` và
`AZURE_STORAGE_ACCOUNT=incomechinh448`.
...
Environment="ARTIFACT_BUCKET=income-lab"
Environment="AZURE_STORAGE_ACCOUNT=incomechinh448"
